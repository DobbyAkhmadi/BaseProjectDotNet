...
function initDataTable() {
  datatable = $('#dataTablePermission').DataTable({
    iDisplayLength: 25,
    processing: true,
    serverSide: true,
    ordering: true,
    responsive: true,
    ajax: {
      url: "/internal/User/Permission/index-post",
      type: 'POST',
    ...
    buttons: [
      ...
      },
      {
        text: '<i class="ti ti-plus me-0 me-sm-1 ti-xs "></i>' +
          '<span class="d-none d-sm-inline-block add-new-permissions">Add New Permission</span>',
...
$("#dataTablePermission").on("click", ".item-delete", function (e) {
  e.preventDefault();
  let url = "/internal/User/Permission/delete"
...
$("#dataTablePermission").on("click", ".item-restore", function (e) {
  e.preventDefault();
  let url = "/internal/User/Permission/restore"
  let dataId = $(this).data('id');
  let form = {
    id: dataId
  }
